...
package ExScaliburGMD::BuildPipelineJob;
...
use ExScaliburGMD::SetBDSenv;
use ExScaliburGMD::PrintBDScfg;
...
sub Build_Pipeline_Job
{
  ...
  my $bds_script = "Run_ExScaliburGMD.bds";
  my $sm = "";
  my $pl = "";
  my $sd = "";
  my $log = "";
  ...
  $job_script = Print_Job($job_script, $bds_cfg_proj, \@aligners, \@callers, \@lines, $retry, $log, $pl, $sd, $bds_script, $proj_dir, $project, $sm);
...
sub Print_Job
{
  my ($job_script, $bds_cfg_proj, $aligners_ref, $callers_ref, $lines_ref, $retry, $log, $pl, $sd, $bds_script, $proj_dir, $project, $sm) = @_;
  ...
  print OUT "\n".join("\n", @$lines_ref)."\n";
  print OUT "\n## submit job \n";
  print OUT "\nbds -c $proj_dir/$bds_cfg_proj -reportHtml -reportYaml -y $retry $log $pl $sd $bds_script -aligners ".join(" ", @$aligners_ref)." -callers ".join(" ", @$callers_ref)." -projdir $proj_dir -project $project $sm > Run_ExScaliburGMD.$project.\$now.log.out 2> Run_ExScaliburGMD.$project.\$now.log.err\n";
