...
package ExScaliburGMD::PrintSample;
...
sub Print_Sample
{
  ...
  foreach my $aligner (sort keys %{$aligner_list}) {
    foreach my $caller (sort keys %{$caller_list}) {
      if($caller eq "gatkhc" || $caller eq "ivc") {
        my $ext = "vcf";
        if($caller eq "gatkhc") { $ext = "gvcf"; }
        print OUT "$aligner\_$caller\_varfile = $proj_dir/results/$project\_samples/$sample/variant_calls/$sample.$aligner.$caller.$ext\n";
      }
    }
  }
