...
package ExScaliburGMD::BuildProjCfg;
...
use ExScaliburGMD::PrintIVC;
use ExScaliburGMD::PrintFlag;
use ExScaliburGMD::PrintSoftware;
use ExScaliburGMD::PrintReference;
use ExScaliburGMD::PrintMultiSample;
...
sub Build_Proj_Cfg
{
  my ($sample_list, $config_list, $proj_dir, $project, $metadata_file, $mapq_min, $threads, $split_flag) = @_;

  my $print_list;
  my $output_dir = "$proj_dir/configs/$project\_project";
  my @keys = qw(ivc flag software reference multisample);
  foreach my $key (sort @keys) { $print_list->{$key} = "$project.$key.cfg"; }  

  ## build project.ivc.cfg
  $config_list = Print_IVC($config_list, $output_dir, $print_list->{"ivc"});

  ## build project.flag.cfg
  $config_list = Print_Flag($config_list, $output_dir, $print_list->{"flag"});

  ## build project.software.cfg
  $config_list = Print_Software($config_list, $output_dir, $print_list->{"software"}, $mapq_min, $threads, $split_flag);
