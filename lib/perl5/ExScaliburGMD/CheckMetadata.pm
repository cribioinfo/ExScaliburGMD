...
package ExScaliburGMD::CheckMetadata;
...
use ExScaliburGMD::Util qw(RGstring);
...
sub Check_Metadata
{
  ...
  foreach my $value (@{$rg_required_ref}) {
    ...
    if($genome eq "") {
      ...
      print STDERR "Check_Metadata: Genome is not in format. Currently supported genomes include hg19. It is \"$genome\"."; 
      $flag = 1;
    }
  if($flavor =~ m/^(\d+)x(\d+)$/) {
