...
package ExScaliburGMD::PrintSoftware;
...
sub Print_Software
{
  my ($config_list, $output_dir, $output_file, $mapq_min, $threads, $split_flag) = @_;
  ...
  if(exists $config_list->{"pipeline"}->{"software"}) {
    ...
    if(exists $config_list->{"pipeline"}->{"software"}->{"main"}) {
      ...
      foreach my $key (sort keys %{$config_list->{"pipeline"}->{"software"}->{"main"}}) {
          ...
          if($key eq "split_by_bin") { $value = $split_flag; }
    ...
    foreach my $key1 (sort keys %{$config_list->{"pipeline"}->{"software"}}) {
      if($key1 ne "main") { 
        ...
        foreach my $key2 (sort keys %{$config_list->{"pipeline"}->{"software"}->{$key1}}) {
          ...
          if($key2 eq "threads") {
            if($key1 =~ m/bwa|fastqc|freebayes|ivc|mpileup|novoalign|picard|pigz|platypus|samtools|gatk/) { $value = $threads; }
