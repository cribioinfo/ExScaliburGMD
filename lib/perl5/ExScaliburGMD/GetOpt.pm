...
our @EXPORT = qw(Get_Opt @aligners @callers $bds_cfg $bds_cfg_proj $command $config_file $config_list $job_script $metadata_file $platform $proj_dir $project $sample_list $scheduler $threads $tree_exe $tree_output $force_flag $log_flag $mapq_min $retry $split_flag);
our @EXPORT_OK = qw(Get_Opt);

## ---------------------------------------

## initialize 
our @aligners = qw(bwaaln bwamem novoalign);
our @callers = qw(gatkhc gatkug freebayes mpileup ivc platypus vcmm);
our $bds_cfg = "";
our $bds_cfg_proj = "";
our $command = "";
our $config_file = "";
...
sub Get_Opt
{
  ...
  $job_script = "Submit_ExScaliburGMD.$project.sh";
...
sub Print_Cmd
{
  ...
  print " ]\n";
  print "Scatter-Gather Mode               = [ ";
  if($split_flag) { print "ON"; } else { print "OFF"; }
  print " ]\n";
