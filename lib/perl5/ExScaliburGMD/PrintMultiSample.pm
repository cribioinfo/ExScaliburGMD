...
package ExScaliburGMD::PrintMultiSample;
...
use ExScaliburGMD::Util qw(Empty_Value Add_Key BAM_Suffix);
...
sub Print_MultiSample
{
  ...
  print OUT "project_report = $proj_dir/report/$project\_report\n";
  ...
  foreach my $aligner (sort keys %{$aligner_list}) {
    print OUT "$aligner\_loci_merged = $proj_dir/results/$project\_multisample/variant_calls/$project.$aligner.target.bed\n";
  }
  ...
  foreach my $aligner (sort keys %{$aligner_list}) {
    foreach my $caller (sort keys %{$caller_list}) {
      print OUT "$aligner\_$caller\_varfile = $proj_dir/results/$project\_multisample/variant_calls/$project.$aligner.$caller.flt.vcf\n";
    }
  }
  ...
  ## -----
  @print_files = ();
  print OUT "\n## gatk haplotypecaller per-sample gvcf files\n";
  foreach my $aligner (sort keys %{$aligner_list}) {
    @print_files = ();
    foreach my $sample (sort keys %{$sample_list}) {
      push(@print_files, "$proj_dir/results/$project\_samples/$sample/variant_calls/$sample.$aligner.gatkhc.gvcf");
    }
    print OUT "$aligner\_gatkhc_gvcffiles = ",join(",", @print_files)."\n";
  }

  ## -----
  @print_files = ();
  print OUT "\n## ivc per-sample vcf files\n";
  foreach my $aligner (sort keys %{$aligner_list}) {
    @print_files = ();
    foreach my $sample (sort keys %{$sample_list}) {
      push(@print_files, "$proj_dir/results/$project\_samples/$sample/variant_calls/$sample.$aligner.ivc.vcf");
    }
    print OUT "$aligner\_ivc_vcffiles = ",join(",", @print_files)."\n";
  }
...
sub Section_Proj 
{
  ...
  my @keys = qw(flag reference software ivc);
  foreach my $key (@keys) {
